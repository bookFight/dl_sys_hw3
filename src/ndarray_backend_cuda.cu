...
struct CudaArray {
  // 构造函数：在CUDA设备上分配size个scalar_t类型的内存
  ...
  // 返回指针的整数表示（可能用于内存地址相关操作）
  size_t ptr_as_int() { return (size_t)ptr; }

  scalar_t* ptr;  // 指向CUDA设备内存的指针
  size_t size;    // 数组元素数量
};

// 存储CUDA核函数的块（block）和网格（grid）维度
struct CudaDims {
  dim3 block, grid;  // dim3是CUDA定义的三维向量类型，用于指定核函数维度
};

// 生成1D计算的CUDA维度配置
CudaDims CudaOneDim(size_t size) {
  CudaDims dim;
  // 计算需要的块数量：向上取整（确保所有元素都被处理）
  size_t num_blocks = (size + BASE_THREAD_NUM - 1) / BASE_THREAD_NUM;
  dim.block = dim3(BASE_THREAD_NUM, 1, 1);  // 每个块包含256个线程（1D）
  dim.grid = dim3(num_blocks, 1, 1);        // 网格大小由块数量决定（1D）
  return dim;
}

#define MAX_VEC_SIZE 8  // 支持的最大维度数（如8维数组）
struct CudaVec {
  uint32_t size;               // 向量维度（如2表示二维）
  int32_t data[MAX_VEC_SIZE];  // 存储各维度的大小（如[3,4]表示3行4列）
};

// 将std::vector转换为CudaVec
...
__device__ size_t indexToMemoryLocation(size_t i, CudaVec shape, CudaVec strides, size_t offset) {
  /**
   * Utility function to convert contiguous index i to memory location from strides
   *
   * Args:
   *   i: contiguous index
   *   shape: vector of shapes of the array
   *   strides: vector of strides of the array
   *   offset: offset of the array   d0 d1 d2 ...dn  i = s0*(d1*d2..dn) + s1*(d2 ...dn) + ...+sn i%dn=sn ,i/=dn
   */
  size_t loc = offset;
  for(int j=shape.size-1; j>=0; j--) {
    loc += (i % shape.data[j]) * strides.data[j];
    i /= shape.data[j];
  }
  return loc;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
   ...
   */
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid >= size) return;  // Ensure we do not access out of
  size_t memLoc= indexToMemoryLocation(gid, shape, strides, offset);
  // Write the value from a to the compacted out array at the calculated memory location
  out[gid]=a[memLoc];
  // Note that this is a simple implementation, and you may want to use shared memory or
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size,
   ...
   * entry in the compact input a, to the corresponding item (at location gid) in the non-compact
   * array out.
   *
   * Args:
   ...
   */
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid >= size) return;  // Ensure we do not access out of bounds
  size_t memLoc = indexToMemoryLocation(gid, shape, strides, offset);
  // Write the value from a to the non-compact out array at the calculated memory location
  out[memLoc] = a[gid];
}
...
__global__ void ScalarSetitemKernel(size_t size, scalar_t val, scalar_t* out, CudaVec shape,
  ...
  if (gid >= size) return;  // Ensure we do not access out of bounds
  size_t memLoc = indexToMemoryLocation(gid, shape, strides, offset);
  // Write the scalar value to the non-compact out array at the calculated memory location
  out[memLoc] = val;
}
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* c, uint32_t M, uint32_t N,
  ...
  int thread_id = thread_x + thread_y * blockDim.x;//当前线程

  int nthreads = blockDim.x * blockDim.y;//所有线程数
  int width=(TILE * TILE+nthreads-1)/nthreads;
...
// 一个block处理TILE*TILE个元素 每个线程处理(TILE * TILE+nthreads-1)/nthreads个元素,,K按TILE被分段,start表示当前分段
  for(int start=0; start<N; start+=TILE){
    ...
    for (int idx = thread_id; idx < TILE * TILE; idx += nthreads){
      int x = idx / TILE; // 在shared中对应元素的索引
      int y = idx % TILE; // 在shared中对应元素的索引
      // a_shared中的(x, y)相当于a中的(x+block_x*TILE, y+start),block*TILE为当前线程块的offset
    ...
    // 接下来把计算V*V的外积结果的要用的数据加载到寄存器数组中,即对于一个线程块计算T*T个元素,设置SA[T][T],SB[T][T],将K方向按T分段,每次循环加载T*T,同理一个线程计算V*V个元素,要使用SA中V*T,SB中T*V,循环T方向,每次加载V个元素
      if(thread_x * V >= TILE || thread_y * V >= TILE)
        continue;
      for(int reg_x=0; reg_x<V; reg_x++){
        int shared_x = reg_x + thread_x * V;  //确定取的share_A x的位置
        if(shared_x >= TILE){
          break;
        }
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN SOLUTION
  dim3 gird_dim=dim3((M+TILE-1) /TILE,(P+TILE-1)/TILE,1);
//   设置二维网格块，对应out矩阵的分块后行列
  dim3 block_dim=dim(16,16,1);
//   每个块内对应有16*16的二维线程块
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t a_size) {
    ...
    // 2. 确定当前线程负责的块的起始和结束索引
    size_t start = gid * size;  // 块起始位置 = 线程索引 × 块大小（size即reduce_size）
    size_t end = min(start + size, a_size);  // 块结束位置（避免越界）

    // 3. 若块有效（起始位置在数组范围内），则计算该块的最大值
    if (start < end) {
        scalar_t max_val = a[start];  // 初始化最大值为块内第一个元素
        // 遍历块内所有元素，更新最大值
        for (size_t i = start + 1; i < end; i++) {
            max_val = max(max_val, a[i]);
        }
        out[gid] = max_val;  // 将块最大值写入输出数组的对应位置
    }
}
