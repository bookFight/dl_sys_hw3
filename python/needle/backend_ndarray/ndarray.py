...
class NDArray:
    ...
    def permute(self, new_axes):
        """
        重新排列数组的维度顺序。new_axes 描述了现有维度的排列方式，例如：
          - 若数组维度为 "BHWC"，则 .permute((0,3,1,2)) 会将其转换为 "BCHW" 顺序。
          - 对于 2D 数组，.permute((1,0)) 会实现数组转置。
        与 reshape 类似，该操作不复制内存，仅通过调整形状和步长实现，新数组与原数组共享内存。

        参数：
            new_axes (tuple)：维度的新排列顺序（由原维度索引组成的元组）

        返回：
            NDArray：维度重排后的新数组，与原数组共享内存（仅修改形状和步长）
        """
        # 验证新维度顺序的合法性：长度必须与原维度数一致，且元素为原维度的索引
        assert len(new_axes) == len(self.shape), "新维度顺序的长度必须与原维度数一致"
        assert set(new_axes) == set(range(len(self.shape))), "新维度顺序必须是原维度索引的排列"

        # 根据新维度顺序，计算新的形状和步长
        new_shape = tuple(self.shape[i] for i in new_axes)
        new_strides = tuple(self.strides[i] for i in new_axes)

        # 调用 as_strided 创建新数组（共享内存，仅更新形状和步长）
    ...
    def __getitem__(self, idxs):
        """
        Python 中的 __getitem__ 运算符允许访问数组元素。当传入类似 a[1:5,:-1:2,4,:] 的索引时，
        Python 会将其转换为切片和整数的元组（例如示例中的 '4' 是整数）。切片可能包含 .start、.stop、.step 三个元素，
        可能为 None 或负值。为简化处理， stub 代码已将索引转换为与数组维度数量一致的切片元组。

        对于该切片元组，返回对应子集的数组。与之前的方法一样，这可通过计算新的形状、步长和偏移量实现，
        新数组指向原数组的内存（不复制）。

        异常：
            若切片大小或步长为负，或切片数量与维度数不匹配，stub 代码已抛出断言错误。

        参数：
            idxs (tuple)：经处理后的切片元组，每个元素对应数组一个维度的子集

        返回：
            NDArray：对应子集的新数组，与原数组共享内存（仅修改形状、步长和偏移量）
        """
        # 处理单元素索引为元组，将整数索引转换为切片（如 4 → slice(4,5,1)）
        ...
        # 验证切片数量与数组维度数一致
        assert len(idxs) == self.ndim, "索引数量必须与维度数相等"

        ### 核心实现
        # 1. 计算新形状：每个维度的元素数量（切片后保留的元素数）
        shape = tuple(
            max(0, (s.stop - s.start + s.step - 1) // s.step)
            for s in idxs
        )
        # 2. 计算新步长：原步长 × 切片步长（反映切片的跨步间隔）
        strides = tuple(
            self.strides[i] * s.step
            for i, s in enumerate(idxs)
        )
        # 3. 计算新偏移量：原偏移量 + 各维度起始索引×原步长的总和（定位子集在原内存的起始位置）
        offset = reduce(
            operator.add,
            (s.start * self.strides[i] for i, s in enumerate(idxs)),
            self._offset  # 初始值为原数组的偏移量
        )
        # 创建新数组（共享原内存，仅更新形状、步长和偏移量）
        return NDArray.make(
            shape,
            strides=strides,
            device=self.device,
